# coding: utf-8
"""Arithmetic operators ``+ - * /`` for the standard profile.

每个算子先做"参数全部为同一 concrete ``NumberValue`` 类型"的一致性检查，
然后把实际算术 dispatch 到对应 number 类型的实现。这条 dispatch 路径
...
def _coerce_host_number(value: object) -> object:
    ...
    Host adapters can deliver raw Python ``int`` / ``float`` to ``+``/``-``/
    etc. (for example, when a Python coroutine consumed a quoted chain and
    handed individual atoms back). The arithmetic kernels work on
    ``NumberValue``s, so promote at the boundary. ``bool`` stays raw — the
    same-type check below will reject it.
    """
